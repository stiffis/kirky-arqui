...
\texttt{riscvtest.txt} mediante \texttt{\$readmemh}. Por eso, cada prueba consiste en un
par \textbf{programa~(\texttt{.txt})~+~testbench~(\texttt{.v})}: se elige el programa de
la familia que se quiere validar, se copia como \texttt{riscvtest.txt} y se compila junto
con su \textit{testbench}. Por ejemplo, para validar los desplazamientos:

\begin{lstlisting}[language={}]
# 1. elegir el programa (lo carga imem via $readmemh)
cp tests/programs/riscvtest_shift.txt riscvtest.txt
# 2. compilar el diseno junto con el testbench de esa familia
iverilog -g2012 -o sim *.v tests/testbench_shift.v
...
\textbf{fallan} (Figura~\ref{fig:hazard-fail}), confirmando que las dependencias no se
resuelven y que la unidad de riesgos es necesaria.

\begin{figure}[ht]
	\centering
	\includegraphics[width=0.85\textwidth]{hazard_fail.png}
	\caption{Salida de \texttt{run\_hazard\_demo.sh}: los programas 2, 3 y 4 fallan al
		ejecutarse sin la unidad de riesgos.}
	\label{fig:hazard-fail}
\end{figure}
...
regresion completa. Todos los casos pasan (17 de 17, incluidos los 12 de la Parte
1 del proyecto):
...
con el valor esperado, confirmando que la extension C funciona correctamente sobre
